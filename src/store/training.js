...
const getters = {
    ...
    trainingMenuDisabled: (state, getters) => {
        ...
        if (state.training.auth === TRAINING_TYPE.Team.name) {
            return !state.intoAccess
        }
    },
    ...
    isTeamTraining: (state, getters) => {
        return state.training.auth === TRAINING_TYPE.Team.name
    },
    // 是否需要显示密码验证框
    ...
    getTraining({ commit, rootState, dispatch }) {
        return new Promise((resolve, reject) => {
            api.getTraining(rootState.route.params.trainingID).then((res) => {
                ...
                } else if (training.auth == TRAINING_TYPE.Team.name) {
                    dispatch('getTrainingAccess', { auth: TRAINING_TYPE.Team.name })
                }
            }, err => {
                reject(err)
            })
    ...
    getTrainingAccess({ commit, rootState }, trainingType) {
        return new Promise((resolve, reject) => {
            api.getTrainingAccess(rootState.route.params.trainingID).then(res => {
                ...
                } else if (trainingType.auth == TRAINING_TYPE.Team.name) {
                    commit('trainingIntoAccess', { intoAccess: res.data.data.access })
                }
                resolve(res)
            }).catch()
