<template>
    <div class="problem-list">
        <vxe-table :data="problemList" align="center" auto-resize border="inner" highlight-hover-row stripe
            @cell-click="goTrainingProblem">
            <vxe-table-column v-if="isAuthenticated" field="status" title="" width="50">
                <template v-slot="{ row }">
            ...
            <vxe-table-column :title="$t('m.Problem_ID')" field="problemId" show-overflow width="150">
            </vxe-table-column>
            <vxe-table-column :title="$t('m.Title')" field="title" min-width="150" show-overflow></vxe-table-column>
            ...
            <vxe-table-column field="tag" min-width="100">
                <template v-slot:header>
                    <el-link v-if="!showTags" :underline="false" type="primary" @click="showTags = !showTags">{{
                        $t('m.Show_Tags') }}
                    </el-link>
                    <el-link v-else :underline="false" type="danger" @click="showTags = !showTags">{{ $t('m.Hide_Tags')
                        }}
                    </el-link>
                </template>
                <template v-slot="{ row }">
                    <div v-if="showTags">
                        <span v-for="tag in row.tags" :key="tag.id" :style="'margin-right:7px;color:#FFF;background-color:' +
                            (tag.color ? tag.color : '#409eff')
                            " class="el-tag el-tag--small">{{ tag.name }}</span>
                    </div>
                </template>
            </vxe-table-column>
                ...
                <template v-slot="{ row }">
                    <span>
                        <el-tooltip :content="row.ac + '/' + row.total" effect="dark" placement="top">
                            <el-progress :percentage="getPassingRate(row.ac, row.total)" :stroke-width="20"
