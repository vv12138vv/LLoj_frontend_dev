        ...
        <el-card>
            <div slot="header">
                ...
                <div class="filter-row">
                    ...
                    <span>
                        <el-button icon="el-icon-plus" size="small" type="primary" @click="goCreateTraining">{{
                            $t('m.Create') }}
                        </el-button>
                    </span>
                </div>
            </div>
            <vxe-table ref="xTable" :data="trainingList" :loading="loading" align="center" auto-resize stripe>
                <vxe-table-column field="id" title="ID" width="80"></vxe-table-column>
                <vxe-table-column :title="$t('m.Order_Number')" field="rank" width="80">
                </vxe-table-column>
                <vxe-table-column :title="$t('m.Title')" field="title" min-width="150" show-overflow>
                </vxe-table-column>
                ...
                <vxe-table-column :title="$t('m.Visible')" min-width="80">
                    <template v-slot="{ row }">
                        <el-switch v-model="row.status" :disabled="!isSuperAdmin && userInfo.username != row.author"
