<template>
    <div class="view">
        <el-card>
            <div slot="header">
                ...
                <div class="filter-row">
                    <span>
                        <el-button icon="el-icon-delete-solid" size="small" type="danger" @click="deleteUsers(null)">{{
                            $t('m.Delete') }}
                        </el-button>
                    </span>
                    <span>
                        <el-button icon="el-icon-minus" size="small" type="warning" @click="forbidUsers(null)">{{
                            $t('m.Forbid') }}
                        </el-button>
                    </span>
                    <span>
                        <vxe-input v-model="query.keyword" :placeholder="$t('m.Enter_keyword')" size="medium"
                            type="search" @search-click="filterByKeyword"
                    ...
                    <!--          </span>-->
                    <span>
                        <el-select v-model="query.userStatus" size="small" style="width: 180px;"
                            @change="userStatusChange">
                            <el-option :label="$t('m.All_User')" :value="-1"></el-option>
                            <el-option v-for="(status, index) in USER_STATUS" :key="index" :label="status.label"
                                :value="status.value"></el-option>
                        </el-select>
                    </span>
                    <span>
                        <el-select v-model="query.userRole" size="small" style="width: 180px;" @change="userRoleChange">
                            <el-option :label="$t('m.All_User')" :value="-1"></el-option>
                            <el-option v-for="(role, index) in USER_ROLE" :key="index" :label="role.name"
                                :value="role.id"></el-option>
                        </el-select>
                    </span>
                    <span>
                        <el-button icon="el-icon-check" size="small" type="primary">{{ $t('m.Pass') }}
                        </el-button>
                    </span>
                    <span>
                        <el-button icon="el-icon-close" size="small" type="warning">{{ $t('m.Reject') }}
                        </el-button>
                    </span>
                </div>
            </div>
            <vxe-table ref="xTable" :checkbox-config="{ labelField: 'id', highlight: true, range: true }"
                :data="userList" :loading="loadingTable" auto-resize stripe @checkbox-change="handleSelectionChange"
                @checkbox-all="handleChangeAll">
                <vxe-column type="checkbox" width="60"></vxe-column>
                <vxe-column field="uid" show-overflow title="UUID" :visible="false"></vxe-column>
                <vxe-column :title="$t('m.User')" field="username" min-width="120" show-overflow></vxe-column>
                <vxe-column :title="$t('m.School')" field="school" min-width="120" show-overflow></vxe-column>
                <vxe-column :title="$t('m.RealName')" field="realname" min-width="90" show-overflow></vxe-column>
                <vxe-column :title="$t('m.Student_Number')" field="number" min-width="120" show-overflow></vxe-column>
                <vxe-column :title="$t('m.Email')" field="email" min-width="170" show-overflow></vxe-column>
                <vxe-column :title="$t('m.Created_Time')" field="gmtCreate" min-width="155">
                    <template v-slot="{ row }">
                        {{ row.gmtCreate | localtime }}
                    </template>
                </vxe-column>
                <vxe-column :title="$t('m.User_Type')" field="role" min-width="100">
                    <template v-slot="{ row }">
                        {{ getRole(row.roles) | parseRole }}
                    </template>
                </vxe-column>
                <vxe-column :title="$t('m.Status')" field="status" min-width="90">
                    <template v-slot="{ row }">
            ...
            <div class="panel-options">
                <el-pagination :page-size="query.pageSize" :current-page.sync="query.currentPage"
                    :page-sizes="[10, 30, 50, 100]" :total="total" class="page" layout="prev, pager, next, sizes"
                    @current-change="currentChange" @size-change="onPageSizeChange">
                </el-pagination>
            </div>
        </el-card>
        ...
        <el-card style="margin-top:20px">
            ...
            <el-upload v-if="!uploadUsers.length" :before-upload="handleUsersCSV" :show-file-list="false" accept=".csv"
                action="">
            ...
            <template v-else>
                <vxe-table :data="uploadUsersPage" auto-resize stripe>
                    <vxe-column :title="$t('m.Username')" field="username" min-width="96" show-overflow>
                        <template v-slot="{ row }">
                            {{ row[0] }}
                        </template>
                    </vxe-column>
                    <vxe-column :title="$t('m.Password')" field="password" min-width="130" show-overflow>
                        <template v-slot="{ row }">
                            {{ row[1] }}
                        </template>
                    </vxe-column>
                    <vxe-column :title="$t('m.Email')" field="email" min-width="120" show-overflow>
                        <template v-slot="{ row }">
                            {{ row[2] }}
                        </template>
                    </vxe-column>
                    <vxe-column :title="$t('m.RealName')" field="realname" min-width="150" show-overflow>
                        <template v-slot="{ row }">
                            {{ row[3] }}
                        </template>
                    </vxe-column>
                    <vxe-column :title="$t('m.Gender')" field="gender" min-width="60" show-overflow>
                        <template v-slot="{ row }">
                            {{ row[4] }}
                        </template>
                    </vxe-column>
                    <vxe-column :title="$t('m.Nickname')" field="nickname" min-width="100" show-overflow>
                        <template v-slot="{ row }">
                            {{ row[5] }}
                        </template>
                    </vxe-column>
                    <vxe-column :title="$t('m.School')" field="school" min-width="100" show-overflow>
                        <template v-slot="{ row }">
                            {{ row[6] }}
                        </template>
                    </vxe-column>
                ...
                <div class="panel-options">
                    ...
                    <el-pagination :current-page.sync="uploadUsersCurrentPage" :page-size="uploadUsersPageSize"
                        :total="uploadUsers.length" class="page" layout="prev, pager, next">
                    </el-pagination>
                </div>
            </template>
        </el-card>
        ...
        <el-card style="margin-top:20px">
            ...
            <el-form ref="formGenerateUser" :model="formGenerateUser" :rules="formGenerateRules">
                <el-row :gutter="10">
                    <el-col :md="5" :xs="24">
                ...
                <el-form-item>
                    <el-button :loading="loadingGenerate" icon="fa fa-users" size="small" type="primary"
                        @click="generateUser">
                        {{ $t('m.Generate_and_Export') }}
                    </el-button>
                    <span v-if="formGenerateUser.number_from <= formGenerateUser.number_to" class="userPreview">
                        ...
                        ">
                            {{
                                ...
                                ("" + formGenerateUser.number_from +
                                    1).padStart(formGenerateUser.number_to.toString().length, "0")
                                +
                            formGenerateUser.suffix +
                            '...'
                            }}
        ...
        <!--编辑用户的对话框-->
        <el-dialog :title="$t('m.User')" :visible.sync="showUserDialog" :close-on-click-modal="false" width="350px">
            <el-form ref="updateUser" :model="selectUser" :rules="updateUserRules" label-position="left"
                label-width="100px">
                <el-row :gutter="10">
                    <el-col :span="24">
                    ...
                    <el-col :span="24">
                        <el-form-item :label="$t('m.Set_New_PWD')">
                            <el-switch v-model="selectUser.setNewPwd" :active-value="true" :inactive-value="false">
                            </el-switch>
                        </el-form-item>
                    </el-col>
                    <el-col v-if="selectUser.setNewPwd == 1" :span="24">
                        <el-form-item :label="$t('m.General_New_Password')" prop="password" required>
                            <el-input v-model="selectUser.password"
                                :placeholder="$t('m.General_New_Password')"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="24">
